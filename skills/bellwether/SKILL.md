...
- The ONLY way to check CI status, PR state, and reviews is `npx bellwether check --watch`. This command blocks until CI completes or the watch times out — do NOT add sleep or polling. Use `--timeout` to extend the wait; never resort to manual polling.
...
```
1. npx bellwether check --watch        (blocks until CI completes or the watch times out; DO NOT substitute with gh/GitHub API calls — use this exact command)
